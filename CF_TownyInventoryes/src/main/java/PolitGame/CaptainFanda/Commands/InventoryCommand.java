        ...
        if(sender instanceof Player p) {
            ...
            if(town != null) {
                if(resident.isMayor() || p.isOp()) {
                    ...
                    String message = prefix+LanguageManager.getMessages().getString("succes-open-inv");
                    message =  message.replace('&', '§');
                    p.sendMessage(message);
                } else {
                    String message = prefix+LanguageManager.getMessages().getString("not-mayor");
                    message = message.replace('&', '§');
                    p.sendMessage(message);
                }
            } else {
                String message = prefix+LanguageManager.getMessages().getString("not-town");
                message = message.replace('&', '§');
                p.sendMessage(message);
            }
        }
